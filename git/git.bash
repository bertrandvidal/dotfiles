...
#           master        compare between the branch your are on and the master
#                         useful for bvidal
...
__git_ps1_show_upstream ()
{
	...
	for option in ${GIT_PS1_SHOWUPSTREAM}; do
		case "$option" in
		git|svn|master) upstream="$option" ;;
		verbose) verbose=1 ;;
		legacy)  legacy=1  ;;
		esac
	done
  ...
  elif [[ "$upstream" -ne "" ]]; then
    current_branch=`git for-each-ref --format=\'%(refname:short)\' `git symbolic-ref HEAD``
    count="$(git rev-list --count --left-right "$upstream"..."$current_branch" 2>/dev/null)"
	else
		...
		if [ "true" = "$(git rev-parse --is-inside-git-dir 2>/dev/null)" ]; then
			...
			if [ -n "${GIT_PS1_SHOWDIRTYSTATE-}" ]; then
				if [ "$(git config --bool bash.showDirtyState)" != "false" ]; then
					git diff --patience --no-ext-diff --quiet --exit-code || w="*"
